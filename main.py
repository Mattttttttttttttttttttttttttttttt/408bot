...
intents.message_content, intents.reactions, intents.members = True, True, True
...
CA_730 = datetime.time(7, 30, 0, tzinfo=CA_TZ)
CA_1220 = datetime.time(12, 20, 0, tzinfo=CA_TZ)
...
OFFICER_ROLE = 1202461788055146517
LIST_408 = [16, 8]
...
#                       user:discord.User|None=None, lb: str|None=None) -> None:
...
#     u = None if user is None else user.id
#     if lb is None:
#         embed_408 = await get_records(u, "408")
#         embed_625 = await get_records(u, "625")
...
#     else:
#         embed: list = await get_records(u, lb)
...
# Storage for effective roles (user_id -> {role_name -> [user_ids]})
EFFECTIVE_ROLES_FILE = "effective_roles.json"

def load_effective_roles():
    """Load effective roles from JSON file"""
    if os.path.exists(EFFECTIVE_ROLES_FILE):
        with open(EFFECTIVE_ROLES_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    return {}

def save_effective_roles(roles_data):
    """Save effective roles to JSON file"""
    with open(EFFECTIVE_ROLES_FILE, 'w', encoding='utf-8') as f:
        json.dump(roles_data, f, indent=4)

effective_roles = load_effective_roles()

# Create a command group called "er"
er_group = app_commands.Group(name="er", description="Effective role management")

@er_group.command(name="create", description="Create a new effective role")
@app_commands.describe(
    name="The name of the effective role",
    users="The users to add (space-separated mentions or IDs)"
)
async def create_effective_role(interaction: discord.Interaction, name: str, users: str):
    """creates an effective role

    Args:
        interaction (discord.Interaction): default parameter
        name (str): the name of the ER
        users (str): the people in it. e.g. "<@1104220935973777459> <@some-other-user-id>"
    """
    user_id = str(interaction.user.id)

    # Initialize user's roles if they don't exist
    if user_id not in effective_roles:
        effective_roles[user_id] = {}

    # Check if role already exists
    if name in effective_roles[user_id]:
        await interaction.response.send_message(
            f"❌ Effective role `{name}` already exists!",
            ephemeral=True
        )
        return
    ...
    if not user_ids:
        await interaction.response.send_message(
            "❌ No valid users provided!",
            ephemeral=True
        )
        return

    # Check which users are in the server
    valid_users = []
    invalid_users = []
    ...
    if not valid_users:
        await interaction.response.send_message(
            "❌ None of the provided users are in this server!",
            ephemeral=True
        )
        return

    # Create the effective role with valid users only
    effective_roles[user_id][name] = valid_users
    save_effective_roles(effective_roles)

    # Build response message
    user_mentions = " ".join([f"<@{uid}>" for uid in valid_users])
    response = f"✅ Created effective role `{name}` with {len(valid_users)} user(s): {user_mentions}"

    if invalid_users:
        invalid_mentions = " ".join([f"<@{uid}>" for uid in invalid_users])
        response += f"\n\n⚠️ The following users are not in this server and were not added: {invalid_mentions}"

    await interaction.response.send_message(response, ephemeral=True)

@er_group.command(name="view", description="View all your effective roles")
async def view_effective_roles(interaction: discord.Interaction):
    """returns an ephemeral message displaying the ERs the user has
    ...
    if user_id not in effective_roles or not effective_roles[user_id]:
        await interaction.response.send_message(
            "You don't have any effective roles yet!",
            ephemeral=True
        )
        return

    # Build the response
    response = "**Your Effective Roles:**\n\n"
    for role_name, user_ids in effective_roles[user_id].items():
        user_mentions = " ".join([f"<@{uid}>" for uid in user_ids])
        response += f"**{role_name}:** {user_mentions}\n"

    await interaction.response.send_message(response, ephemeral=True)

@er_group.command(name="ping", description="Ping an effective role you created")
@app_commands.describe(name="The name of the effective role to ping")
async def ping_effective_role(interaction: discord.Interaction, name: str):
    """returns an ephemeral message in the format of f"@this_is_not_matt @{some_other_username}"

    Args:
        interaction (discord.Interaction): default parameter
        name (str): the name of the ER to fetch
    """
    user_id = str(interaction.user.id)

    if user_id not in effective_roles or name not in effective_roles[user_id]:
        await interaction.response.send_message(
            f"❌ Effective role `{name}` not found!",
            ephemeral=True
        )
        return

    user_ids = effective_roles[user_id][name]
    ...
    for uid in user_ids:
        member = interaction.guild.get_member(int(uid))
        if member:
            valid_users.append((uid, member.name))
        else:
            invalid_users.append(uid)

    # Build response message with @username format
    if valid_users:
        response = " ".join([f"@{username}" for uid, username in valid_users])
    else:
        response = f"**{name}:** No valid users in this server."

    if invalid_users:
        invalid_mentions = " ".join([f"<@{uid}>" for uid in invalid_users])
        response += f"\n\n⚠️ The following users are not in this server: {invalid_mentions}"

    await interaction.response.send_message(response, ephemeral=True)

@er_group.command(name="add", description="Add users to an effective role")
@app_commands.describe(
    name="The name of the effective role",
    users="The users to add (space-separated mentions or IDs)"
)
async def add_to_effective_role(interaction: discord.Interaction, name: str, users: str):
    """add some users to an ER

    Args:
        interaction (discord.Interaction): default parameter
        name (str): the name of the ER to add to
        users (str): the people to add
    """
    user_id = str(interaction.user.id)

    if user_id not in effective_roles or name not in effective_roles[user_id]:
        await interaction.response.send_message(
            f"❌ Effective role `{name}` not found! This command is for adding people to an existing effective role.",
            ephemeral=True
        )
        return
    ...
    # Check which users are in the server and not already in the role
    added = []
    already_in_role = []
    not_in_server = []

    for uid in user_ids:
        if uid in effective_roles[user_id][name]:
            already_in_role.append(uid)
        else:
            member = interaction.guild.get_member(int(uid))
            if member:
                effective_roles[user_id][name].append(uid)
                added.append(uid)
            else:
                not_in_server.append(uid)
    ...
    if added:
        user_mentions = " ".join([f"<@{uid}>" for uid in added])
        response_parts.append(f"✅ Added {len(added)} user(s) to `{name}`: {user_mentions}")

    if already_in_role:
        if not added:
            response_parts.append(f"ℹ️ All specified users were already in `{name}`")

    if not_in_server:
        invalid_mentions = " ".join([f"<@{uid}>" for uid in not_in_server])
        response_parts.append(
            f"⚠️ The following users are not in this server and were not added: {invalid_mentions}"
        )

    if not response_parts:
        response = f"ℹ️ No changes made to `{name}`"
    else:
        response = "\n\n".join(response_parts)

    await interaction.response.send_message(response, ephemeral=True)

@er_group.command(name="remove", description="Remove users from an effective role")
@app_commands.describe(
    name="The name of the effective role",
    users="The users to remove (space-separated mentions or IDs)"
)
async def remove_from_effective_role(interaction: discord.Interaction, name: str, users: str):
    """remove some people from an ER

    Args:
        interaction (discord.Interaction): default parameter
        name (str): the name of the ER to modify
        users (str): the people to remove
    """
    user_id = str(interaction.user.id)
    ...
    for uid in user_ids:
        if uid in effective_roles[user_id][name]:
            effective_roles[user_id][name].remove(uid)
            removed.append(uid)

    save_effective_roles(effective_roles)

    if removed:
        user_mentions = " ".join([f"<@{uid}>" for uid in removed])
        await interaction.response.send_message(
            f"✅ Removed {len(removed)} user(s) from `{name}`: {user_mentions}",
            ephemeral=True
        )
    else:
        await interaction.response.send_message(
            f"ℹ️ None of the specified users were in `{name}`",
            ephemeral=True
        )

@er_group.command(name="delete", description="Delete an effective role")
@app_commands.describe(name="The name of the effective role to delete")
async def delete_effective_role(interaction: discord.Interaction, name: str):
    """delete an effective role

    Args:
        interaction (discord.Interaction): default parameter
        name (str): the name of the ER to delete
    """
    user_id = str(interaction.user.id)
    ...
    del effective_roles[user_id][name]
    save_effective_roles(effective_roles)

    await interaction.response.send_message(
        f"✅ Deleted effective role `{name}`",
        ephemeral=True
    )

# Add the group to the bot's command tree
bot.tree.add_command(er_group)
...
# pings @408 ping at 4:08pm PST/PDT
...
# pings @625 ping at 6:25pm PST/PDT
